...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Obstacle'] and types['Player'] then
            self.launchMarker.collision = true
...
function Level:update(dt)
    ...
    if self.launchMarker.alien ~= nil then
        for i = #self.launchMarker.alien, 1, -1 do
            if self.launchMarker.launched then
                local xPos, yPos = self.launchMarker.alien[i].body:getPosition()
                local xVel, yVel = self.launchMarker.alien[i].body:getLinearVelocity()
                
                -- if we fired our alien to the left or it's almost done rolling, respawn
                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                    self.launchMarker.alien[i].body:destroy()
                    if self.launchMarker.aliens == nil then
                        self.launchMarker = AlienLaunchMarker(self.world)
                    end
