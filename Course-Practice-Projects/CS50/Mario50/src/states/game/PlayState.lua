...
function PlayState:enter(params)
    ...
    -- Code the Ensures that Player is never spawned on Chasm
    for x = 0, VIRTUAL_WIDTH do
        local shift = true
        for y = 7, VIRTUAL_HEIGHT do
            local CheckTile = self.tileMap:pointToTile(x,y)
            if CheckTile.id == TILE_ID_GROUND then
                shift = false
                break
            end
        end
        if shift == false and x > 0 then 
            self.player.x = x - 1
            self.player.y = 0
            break
        elseif shift == false and x == 0 then
            self.player.x = 0
        end
    end
