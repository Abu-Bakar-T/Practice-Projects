...
function LevelMaker.createMap(level)
    ...
    local highestColor = math.min(6, level % 6 + 3)

    -- lay out bricks such that they touch each other and fill the space
    for y = 1, numRows do
        -- Flag for Locked Brick
        local lockedbrickFlag = false;
        ...
        -- setting tier for Black Color
        if alternateColor1 == 6 then
            alternateTier1 = math.random(0,1)
            if(alternateTier1 == 1) then
                lockedbrickFlag = true
                alternateTier1 = 0
            end
        end
        if alternateColor2 == 6 then
            alternateTier2 = math.random(0,1)
            if(alternateTier2 == 1) then
                lockedbrickFlag = true
                alternateTier2 = 0
            end
        end
        ...
        for x = 1, numCols do
            ...
            if alternatePattern and lockedbrickFlag then
                b.color = 6
                b.tier = 1
                lockedbrickFlag = false
